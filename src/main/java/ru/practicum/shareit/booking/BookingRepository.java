...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
    ...
    @Query(value = "select * from bookings as b\n" +
            ...
            "order by b.start_time DESC", nativeQuery = true)
    List<Booking>findCurrentOwnerBookings(Integer user_id, LocalDateTime localDateTime);

    @Query("select b from Booking b where b.item.id = ?1 and b.start >= ?2 and b.end <= ?2 and b.status = 'APPROVED'")
    List<Booking>havingBookingsAtCurrentDate(Integer item_id, LocalDateTime start);
    ...
    List<Booking>findAllByBooker_IdAndStatusAndEndBefore(Integer user_id, Status status, LocalDateTime localDateTime);

    @Query(value = "select * from bookings as b where user_id = ?1 and start_time <= ?2  " +
            "and end_time > ?2 order by end_time DESC",
            nativeQuery = true)
    List<Booking>findCurrentBookings(Integer user_id, LocalDateTime localDateTime);
